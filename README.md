...
- **Shop → Update** (or the automatic first run) crawls the stores for every food in your plan. It finds products in the **sitemaps** the stores publish for crawlers (about 10,000 food products at Pingo Doce and 28,000 at Auchan). A product counts only if its name really is that food **and** it sits on that food's aisle in the store (the category in its address; Mercadona's own categories). So lemon iced tea, a pineapple infusion or banana chips never stand in for the fruit. Then it reads the best few product pages, preferring the store's own brand and non-organic. From each page it saves the photo (400 px), name, price, price per kg, pack size, barcode and the **nutrition label**, in `data/products/`. Photos are served by Leve itself, so they also work offline in the supermarket.
- **Prices** are read the way the shelf shows them. Weighed products (meat, fish, fruit) are priced per kg with the usual piece or tray (Auchan: "Quant. Mínima = 500g"; Pingo Doce: "1 Un = 0,5 kg"). Packs come with their size and price per kg, litre or item.
- **Nutrition labels** are read from the store page. Pingo Doce shows them as a table, Auchan as one sentence, and both are handled. Some pages leave a row out: Pingo Doce drops rows that are 0 (no carbohydrate row for fish), and a few Auchan pages have no carbohydrate row. When only carbohydrate or only fat is missing, it is worked out from the label's own energy and marked in the product sheet. When a page has no label but has a barcode, the label comes from **Open Food Facts** for that exact barcode, which is how Mercadona products get theirs. Fresh meat, fish and loose fruit and vegetables have no label (the law doesn't require one), so they use CIQUAL.
...
- **Product labels:** after **Shop → Update**, the label of the product you buy replaces the reference for that food. It is used only if its energy matches its macros and it is no more than 40% below or 60% above the reference (or within 25 kcal of it, for low-energy foods such as unsweetened almond drink), which catches per-portion columns and misread tables. You can turn this off in **Settings → Food preferences**. Each product sheet shows your plan's values, the label and CIQUAL side by side.
